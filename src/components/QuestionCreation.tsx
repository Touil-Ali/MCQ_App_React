...
const QuesitonCreation: React.FC = () => {
  ...
  const handleAddSubQuestion = (index: number) => {
    const updatedQuestions = [...questions];
    updatedQuestions[index].subQuestions.push({ content: '', isCorrect: false });
    setQuestions(updatedQuestions);
  };
  ...
  const handleSubQuestionChange = (qIndex: number, sqIndex: number, value: string, isCorrect: boolean) => {
    const updatedQuestions = [...questions];
    updatedQuestions[qIndex].subQuestions[sqIndex] = { content: value, isCorrect };
    setQuestions(updatedQuestions);
  };
  const handleCreationQcm = async () => {
    try {
      ...
      for (const question of questions) {
        ...
        const response = await fetch(`http://localhost:8080/qcms/${qcmId}/questions`, {
          method: 'POST',
          headers: {
          ...
          {question.subQuestions.map((subQuestion, sqIndex) => (
            <div key={sqIndex}>
              <TextField id="outlined-basic"
                ...
                onChange={(e) => handleSubQuestionChange(qIndex, sqIndex, e.target.value, subQuestion.isCorrect)}
              />
              <label>
                Correct Answer:
                <input
                  ...
                  onChange={(e) => handleSubQuestionChange(qIndex, sqIndex, subQuestion.content, e.target.checked)}
                />
              </label>
              {question.subQuestions.length > 1 && (
                <Button variant="contained" color="error" onClick={() => handleRemoveSubQuestion(qIndex, sqIndex)}>Remove Sub-Question</Button>
              )}
            </div>
          ))}
          <Button variant="contained" onClick={() => handleAddSubQuestion(qIndex)}>Add Sub-Question</Button>
          {questions.length > 1 && (
            <Button variant="contained" color="error" onClick={() => handleRemoveQuestion(qIndex)}>Remove Question</Button>
          )}
        </div>
      ))}
      <Button variant="contained" onClick={handleAddQuestion}>Add Question</Button>
      <Button variant="contained" color="success" onClick={handleCreationQcm}>Create QCM</Button>
    </div>
  )
}
